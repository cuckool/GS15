...
def ecb_encryption(plain_text, enc_key, encryption_function):
    """
    Une entrée de la liste plain_text >> un bloc de 64 bits
    """
    cypher_txt = []
    for i in plain_text:
        cypher_txt.append(encryption_function(enc_key, i))
    return cypher_txt


def ecb_decryption(cypher_txt, dec_key, decryption_function):
    plain_text = []
    for i in cypher_txt:
        plain_text.append(decryption_function(dec_key, i))
    return plain_text


def cbc_encryption(plain_text, enc_key, encryption_function):
    """
    ...
    for i in plain_text_cp:
        ...
        cypher_bloc = encryption_function(enc_key, temp)
        initialisation_vector = cypher_bloc
        cypher_txt.append(cypher_bloc)
    return cypher_txt


def cbc_decryption(cypher_txt, dec_key, decryption_function):
    plain_text = []
    for ind, i in enumerate(cypher_txt[1:]):
        temp = decryption_function(dec_key, i)
...
def pcbc_encryption(plain_text, enc_key, encryption_function):
    """
    Une entrée de la liste plain_text >> un bloc de 64 bits
    """
    cypher_txt = []
...
def pcbc_decryption(cypher_txt, dec_key, decryption_function):
    """
        Une entrée de la liste plain_text >> un bloc de 64 bits
    """
    plain_text = []
    ...
    for ind, block in enumerate(cypher_txt):
        temp = decryption_function(dec_key, block)
...
def cfb_encryption(plain_text, enc_key, encryption_function):
    """
    Retourne une list avec un nb de plus en [0], le bloc qui ne pourra pas être décodé sans l'IV
    Une entrée de la liste plain_text >> un bloc de 64 bits
    """
    ...
    for block in plain_text_cp:
        temp = encryption_function(enc_key, initialisation_vector)
...
def cfb_decryption(cypher_txt, dec_key, encryption_function):
    plain_text = []
    for ind, i in enumerate(cypher_txt[1:]):
        temp = encryption_function(dec_key, cypher_txt[ind]) #ça devrait être ind-1 mais, enumerate est sur cypher_txt[1:], donc le index de cet enumerate est le ind-1 de enumerate(cypher)
        res = temp ^ i
        plain_text.append(res)
    return plain_text
